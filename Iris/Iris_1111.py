...
x_train = iris_train[:, :4]  # 提取训练集花萼长度、花萼宽度、花瓣长度、花瓣宽度属性
y_train = iris_train[:, 4]
num_train = len(x_train)

iris_test = np.array(df_iris_test)
x_test = iris_test[:, :4]  # 提取测试集花萼长度、花萼宽度、花瓣长度、花瓣宽度属性
y_test = iris_test[:, 4]
num_test = len(x_test)
...
# 设置模型参数初始值
np.random.seed(612)
# 权值矩阵W   偏置项B   W 5行3列2维张量，正态分布的随机值
...
# M = 500
# x1_min, x2_min, x3_min = x_train.min(axis=0)
# x1_max, x2_max, x3_max = x_train.max(axis=0)
# t1 = np.linspace(x1_min, x1_max, M)
# t2 = np.linspace(x2_min, x2_max, M)
# t3 = np.linspace(x3_min, x3_max, M)
# m1, m2 = np.meshgrid(t1, t2)
# m1, m3 = np.meshgrid(t1, t3)
# m2, m3 = np.meshgrid(t2, t3)
#
# m0 = np.ones(M * M)
# X_ = tf.cast(np.stack((m0, m1.reshape(-1), m2.reshape(-1), m3.reshape(-1)), axis=1), tf.float32)
...
# ax.scatter(x_train[:, 0], x_train[:, 1], x_train[:, 2], c=y_train, cmap='brg')
...
# cm_bg = mpl.colors.ListedColormap(['#A0FFA0', '#FFA0A0', '#A0A0FF'])
# ax = plt.subplot(projection='3d')
# ax.scatter(x_test[:, 0], x_test[:, 1], x_test[:, 2], c=y_test, cmap='brg')
# ax.set_xlabel('花萼-width')
# ax.set_ylabel('花瓣-length')
# ax.set_zlabel('花瓣-width')
